...
@SuppressWarnings({"SameNameButDifferent", "AvoidCommonTypeNames"})
public sealed interface Hypercube<T> extends AutoCloseable {

  /**
   * Creates a new {@link Hypercube} backed by data in the provided {@link NetcdfFile} and
   * generating objects based on the field bindings provided by the {@link SchemaBinding}.
   *
  ...
  /**
   * Creates an iterator that will return all records in the {@link Hypercube} traversing each
   * dimension in canonical order (left->right, top->bottom).
   */
  Iterator<T> iterator();

  /**
   * Provided for fluency, this allows us to transform our {@link Hypercube} into one with a
   * different generic type without leaving the {@link Hypercube} API behind.
   *
   * <p>This is useful as most {@link Hypercube} implementations target mutable types out-of-the-box
   * in their generics for performance reasons, e.g. those built via {@link SchemaBinding}.
   *
   * <p>This allows us to convert those mutable cubes into ones that return immutable values, such
   * as the result of a {@code Builder.build()} method, presenting it as a cube of immutable data.
   */
   ...
   *
   * <p>The default implementation uses a {@link Spliterator} of unknown size, implementations may
   * want to overwrite this if they know the shape of the iterator.
   */
  ...
  non-sealed interface D1<T> extends Hypercube<T> {
    ...
    default Stream<T> stream() {
      return StreamSupport.stream(
          Spliterators.spliterator(iterator(), max(), Spliterator.SORTED), false);
    }
  ...
  non-sealed interface D2<T> extends Hypercube<T> {
    ...
    default Stream<T> stream() {
      return StreamSupport.stream(
          Spliterators.spliterator(iterator(), (long) d0Max() * d1Max(), Spliterator.SORTED),
          false);
    }
    ...
    default Stream<T> stream() {
      return StreamSupport.stream(
          Spliterators.spliterator(
              iterator(), (long) d0Max() * d1Max() * d2Max(), Spliterator.SORTED),
          false);
    }
  ...
  non-sealed interface D4<T> extends Hypercube<T> {
    ...
    default Stream<T> stream() {
      return StreamSupport.stream(
          Spliterators.spliterator(
              iterator(), (long) d0Max() * d1Max() * d2Max() * d3Max(), Spliterator.SORTED),
          false);
    }
  ...
  /**
   * Dedicated runtime exception class for issues encountered when streaming records out of a netcdf
   * file.
   */
