...
/**
 * Internal type representing the schema of an {@link Array} index to an object of type 'T' via a
 * {@link FieldBinding}.
 *
 * <p>The goal of this interface is to hide the underlying type of the field that's being bound
 * (e.g. an int, float, etc.) from the code performing the index-based schema to create an object.
 */
sealed interface IndexBinding<T> {

  /**
   * Wraps the provided {@link IndexBinding} as one that fails quietly if {@link IOException}s are
   * encountered during calls to the set method.
   *
