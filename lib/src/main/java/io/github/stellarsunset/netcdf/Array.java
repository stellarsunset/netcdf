...
/**
 * This interface hides the NetCDF {@link ucar.ma2.Array} type from the code in the repo adding two
 * features we want when using these downstream.
 *
 * <ol>
 *   <li>Sealing - we want compile-time errors if any of the library code doesn't handle some new
 *       array primitive type
 *   <li>Re-Organization - this re-organizes the Array class hierarchy to put dimensionality before
 *       type, the inverse of in the underlying ucar library, this makes some operations we want to
 *       do in-repo, simpler
...
sealed interface Array {
  ...
  /**
   * Wrap the provided UCAR array instance as sealed array type in the codebase to simplify
   * interactions between types and dimensions of arrays in-repo.
   */
  ...
  static <S, T extends S> T getAsOrThrow(S s, Class<T> clz) {
    ...
    var message = String.format("Unable to downcast %s to %s", s.getClass(), clz);
