 ...
 *
 * <p>This class introduces the concept of two different classes of variables clients may want to
 * bind into Java objects:
 *
 * <ol>
 *   <li>Dimension Variables - these are 1D variables that vary directly along a single dimension
 *   <li>Coordinate Variables - these are (often) multi-dimensional variables that are have entries
 *       for each coordinate in a multi-dimensional grid
 * </ol>
 *
 ...
 * }</pre>
 *
 * <p>The above maps to a 10x10x10 cube where each x coordinate has a corresponding latitude, y
 * coordinate has a longitude, and z coordinate has a "level" denoting some height above ground.
 * Then there are measurements or fields available at each coordinate in the grid e.g. temperature
 * and pressure.
 *
 ...
 *
 * <ol>
 *   <li>All coordinate variables share the same common dimensions
 *   <li>Dimension variables match at least one of the shared coordinate variable dimensions
 * </ol>
 *
 * <p>Note this schema instance may also be used directly with an {@link OutputStream} template type
 * to directly sink records to some outbound data stream.
 */
public final class SchemaBinding<T> {
  ...
  public static final class Builder<T> {
    ...
    /**
     * Initialization method for getting a new instance of a record of type {@link T} that the
     * dimension/coordinate variable schema operations can then be called against.
     *
     * <p>Usually this is some form of {@code new T.Builder();}, but for bindings that directly pipe
     * fields into an {@link OutputStream} this may simply return the same shared output stream
     * instance.
     *
     * @param recordInitializer called to initialize a new record to bind the netcdf-provided fields
     *     into
     */
    ...
    /**
     * Configure an object schema for the provided dimension variable varying across the given
     * dimension, e.g. the latitude variable varying along the 'x' dimension.
     *
     * <p>See the top-level Javadocs for a full description.
     *
     * <p>This is a less type-safe version of the well-named setters that may be more suitable in
     * some situations.
     *
     ...
     */
    public Builder<T> dimensionVariable(
        String dimensionName, String variableName, FieldBinding<T> setter) {
      this.dimensionToVariables.put(dimensionName, variableName);
      this.dimensionVariables.put(variableName, setter);
      return this;
    }

    public Builder<T> byteDimensionVariable(
        String dimensionName, String variableName, FieldBinding.Byte<T> setter) {
    ...
    public Builder<T> booleanDimensionVariable(
        String dimensionName, String variableName, FieldBinding.Bool<T> setter) {
      return dimensionVariable(dimensionName, variableName, setter);
    }
    ...
    public Builder<T> intDimensionVariable(
        String dimensionName, String variableName, FieldBinding.Int<T> setter) {
      return dimensionVariable(dimensionName, variableName, setter);
    }
    ...
    public Builder<T> floatDimensionVariable(
        String dimensionName, String variableName, FieldBinding.Float<T> setter) {
      return dimensionVariable(dimensionName, variableName, setter);
    }
    ...
    /**
     * Optional finalization operation that well be called under the hood before the record is made
     * available in the {@link Stream} output of the {@link NetcdfRecordReader}.
     *
     * <p>This hook is primarily useful when the template type of the schema is flavor of {@link
     * OutputStream} or a {@link Writer} implementation and data returned from the file is being
     * directly re-written to that stream but may need a record termination indicator, e.g. a
     * closing brace for a JSON-based output stream or a newline for a plain text based one.
     *
     * @param recordFinalizer "finalizer" operation to run after the various variable bindings have
     *     been invoked
     */
