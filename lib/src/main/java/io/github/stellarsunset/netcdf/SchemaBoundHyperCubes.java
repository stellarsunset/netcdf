...
class SchemaBoundHyperCubes {
  ...
  /**
   * Creates a new {@link Hypercube} using the {@link ValidatedBinding} that can be queried by index
   * (i.e. x,y,z) for records of type {@code T}.
   *
   * @param binding the validated schema binding containing the field bindings and paired {@link
   *     NetcdfFile}
   */
    ...
    return switch (binding) {
      case ValidatedBinding.D0<T> d0 ->
          throw new IllegalArgumentException(
              "D0 hypercubes aren't supported... why would you need one?");
  ...
  private static <T> Hypercube.D1<T> makeD1(ValidatedBinding.D1<T> binding) {
    ...
    IndexBinding.D1<T> coordinates =
        schema.coordinateVariables().entrySet().stream()
            .map(
                entry ->
                    createD1Binding(
                        requireNonNull(
                            file.findVariable(entry.getKey()),
                            "Missing required variable, check validation logic."),
                        entry.getValue()))
            .reduce(IndexBinding.D1.noop(), IndexBinding.D1::combine);

    IndexBinding.D1<T> dimension =
        combineDimensionVariableBindings(file, schema.dimensionVariables());
  ...
  private static <T> IndexBinding.D1<T> combineDimensionVariableBindings(
      NetcdfFile file, Map<String, FieldBinding<T>> dimensionVariables) {
        ...
        .map(
            entry ->
                createD1Binding(
                    requireNonNull(
                        file.findVariable(entry.getKey()),
                        "Missing required dimension, check validation logic."),
                    entry.getValue()))
        .reduce(IndexBinding.D1.noop(), IndexBinding.D1::combine);
  }

  private static <T> IndexBinding.D1<T> createD1Binding(
      Variable variable, FieldBinding<T> fieldBinding) {
    try {
      ...
      throw new IllegalStateException(
          String.format(
              "Error reading variable %s's data from underlying file.", variable.getFullName()),
          e);
    }
  }
  ...
  private static <T> IndexBinding.D2<T> createD2Binding(
      Variable variable, FieldBinding<T> fieldBinding) {
    try {
  ...
  private static <T> IndexBinding.D3<T> createD3Binding(
      Variable variable, FieldBinding<T> fieldBinding) {
    try {
  ...
  private static <T> IndexBinding.D4<T> createD4Binding(
      Variable variable, FieldBinding<T> fieldBinding) {
    try {
  ...
  /**
   * 1-Dimensional hypercube implementation, meant for reading simple linear variables (e.g. time)
   * from the underlying NetCDF file standalone.
   *
   * <p>Prefer instantiation via {@link Hypercube#schemaBound(NetcdfFile, SchemaBinding)}.
   */
  record D1<T>(
      Supplier<T> initializer,
      IndexBinding.D1<T> d0Setter,
      int max,
      IndexBinding.D1<T> coordinatesSetter,
      Consumer<T> finalizer)
      implements Hypercube.D1<T> {
  ...
  record D2<T>(
      ...
      IndexBinding.D1<T> d0Setter,
      int d0Max,
      IndexBinding.D1<T> d1Setter,
      int d1Max,
      IndexBinding.D2<T> coordinatesSetter,
      Consumer<T> finalizer)
      implements Hypercube.D2<T> {
  ...
  record D3<T>(
      ...
      IndexBinding.D1<T> d1Setter,
      int d1Max,
      IndexBinding.D1<T> d2Setter,
      int d2Max,
      IndexBinding.D3<T> coordinatesSetter,
      Consumer<T> finalizer)
      implements Hypercube.D3<T> {

    @Override
    public T read(int i0, int i1, int i2) {
      T r =
          coordinatesSetter.set(
              d0Setter.set(d1Setter.set(d2Setter.set(initializer.get(), i2), i1), i0), i0, i1, i2);
      finalizer.accept(r);
      return r;
    }
  ...
  record D4<T>(
      ...
      IndexBinding.D1<T> d2Setter,
      int d2Max,
      IndexBinding.D1<T> d3Setter,
      int d3Max,
      IndexBinding.D4<T> coordinatesSetter,
      Consumer<T> finalizer)
    ...
    public T read(int x, int y, int z, int t) {
      T r =
          coordinatesSetter.set(
              d0Setter.set(d1Setter.set(d2Setter.set(d3Setter.set(initializer.get(), t), z), y), x),
              x,
              y,
              z,
              t);
