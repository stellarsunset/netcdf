...
/**
 * Wrapper for a {@link SchemaBinding} instance that has been validated as consistent with the
 * provided NetCDF file, and is therefore valid for reading sub-schema records from it.
 *
 * <p>Validated bindings include:
 *
 * <ol>
 *   <li>The inferred dimensionality of the coordinate data being read so downstream classes can use
 *       that information to generate appropriate {@link Hypercube}, etc. objects
 *   <li>A close method, as they require opening the file to validate the schema contents and
 *       there's no real reason to re-close and re-open it downstream.
 * </ol>
 */
sealed interface ValidatedBinding<T> extends AutoCloseable {

  /**
   * Validate the provided {@link SchemaBinding} against the data in the provided {@link NetcdfFile}
   * returning a handle for a validated schema downstream classes can have good expectations against
  ...
  private static <T> Variable anyVariable(ValidatedBinding<T> binding) {
    Variable anyVariable =
        binding
            .context()
            .findVariable(
                binding.schema().coordinateVariables().entrySet().iterator().next().getKey());
   ...
   *
   * <p>This is done as a private static method because I don't think we actually want to expose
   * this on the interface directly, as in the D0 case it doesn't really make sense.
   *
  ...
  private static <T> Map<String, FieldBinding<T>> dimensionVariables(
    ...
    return binding.schema().dimensionVariables().entrySet().stream()
        .filter(
            entry ->
                requireNonNull(binding.context().findVariable(entry.getKey()))
                    .getDimension(0)
                    .equals(dimension))
  ...
  record Validator<T>(NetcdfFile file, SchemaBinding<T> schema) {
    ...
    /**
     * Optionally return an {@link Error} describing any issues which would result in errors if the
     * provided schema was run against the provided {@link NetcdfFile}.
     */
    Either<ValidatedBinding<T>, Error> validate() {
      // this order does matter, if the variable exists then the dimensions do too, saving us some
      // checks
      return checkVariables()
          .map(Either::<ValidatedBinding<T>, Error>ofRight)
          .orElseGet(this::checkDimensions);
    }
    ...
    private Optional<Error> checkVariables() {
      ...
      for (Map.Entry<String, FieldBinding<T>> coordinateVariable :
          schema.coordinateVariables().entrySet()) {
      ...
      for (Map.Entry<String, FieldBinding<T>> dimensionVariable :
          schema.dimensionVariables().entrySet()) {
    ...
    /**
     * Check a requested variable exists and its setter type handles the {@link DataType} associated
     * with that variable in the provided file.
     */
    ...
    private Optional<Error> checkType(Variable variable, FieldBinding<T> setter) {
      return switch (variable.getDataType()) {
        ...
        case STRING, STRUCTURE, SEQUENCE, OPAQUE, OBJECT ->
            Optional.of(
                new Error.UnhandledVariableType(variable.getFullName(), variable.getDataType()));
      };
    }

    @SuppressWarnings("rawtypes")
    private Optional<Error> shouldBe(
        Variable variable, FieldBinding<T> setter, Class<? extends FieldBinding> expected) {
      if (!expected.isAssignableFrom(setter.getClass())) {

        // Runtime class can be a lambda... need to map it to a meaningful name
        Class<? extends FieldBinding> setterType =
            switch (setter) {
        ...
        Error error =
            new Error.IncorrectVariableType(
                variable.getFullName(), variable.getDataType(), setterType);
    ...
    private Either<ValidatedBinding<T>, Error> checkDimensions() {
      ...
      for (String variableName : schema.coordinateVariables().keySet()) {
        ...
        dimensionsToCoordinateVariables.put(makeDimensionKey(variable), variableName);
        ...
        if (!dimensions.contains(dimension.getName())) {
          builder.addError(
              new Error.MismatchedDimensionVariableDimensions(
                  variableName, dimension.getName(), dimensions));
        }
      }
      ...
      return switch (dimensions.size()) {
        ...
        default ->
            Either.ofRight(
                new Error.UnsupportedCoordinateVariableDimensionality(
                    dimensions.size(), schema.coordinateVariables().keySet()));
      };
    }
    ...
    /**
     * Sealed hierarchy of error modes when comparing bindings to files to allow downstream
     * consumers to switch over each type safely and potentially handle/correct them.
     */
    ...
    sealed interface Error {
      ...
      /**
       * Indicates the {@link DataType} of the requested variable is not supported for object
       * schema.
       */
      ...
      /**
       * Indicates the configured field setter for the requested variable does not match the
       * declared {@link DataType} of the variable's data in the file.
       */
      @SuppressWarnings("rawtypes")
      record IncorrectVariableType(
          String variableName, DataType fileType, Class<? extends FieldBinding> setterType)
          implements Error {
        @Override
        public IllegalArgumentException asException() {
      ...
      record MismatchedCoordinateVariableDimensions(
        ...
        public RuntimeException asException() {
          ...
          StringBuilder builder =
              new StringBuilder().append("Coordinates do not vary over the same dimensions...\n");

          for (Map.Entry<String, Collection<String>> entry :
              dimensionsToCoordinateVariables.asMap().entrySet()) {
      ...
      /**
       * Indicates the given dimension variable varies over a dimension that doesn't match any of
       * the dimensions the requested coordinate variables vary over.
       */
      record MismatchedDimensionVariableDimensions(
          String variableName, String dimensionName, Set<String> coordinateDimensions)
          implements Error {
        @Override
        public RuntimeException asException() {
      ...
      /**
       * Indicates the given dimension variable doesn't vary over a single dimension, and as such
       * should be a coordinate variable.
       *
       * <p>This library doesn't currently support arbitrary sub-dimensionality for dimension
       * variables, they must be scalar.
       */
      record NonScalarDimensionVariable(String variableName, String dimensionNames)
          implements Error {
      ...
      /**
       * Indicates coordinate variables of the given number of dimension are not currently
       * supported.
       */
      record UnsupportedCoordinateVariableDimensionality(
          int dimensionCount, Set<String> variableNames) implements Error {
